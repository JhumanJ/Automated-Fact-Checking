...
"""
def computeIDF(words,invertedIndex,documentCount,idToIgnore=None):
    ...
    for word in words:
        score[word] = math.log10(float(documentCount)/len([item for item in invertedIndex[word]]))
...
"""
def computeTfIdf(text,id,invertedIndex,documentCount,ignoreId=False):

    words = splitWords(text)

    tf = computeTf(words)
    idf = computeIDF(words, invertedIndex, documentCount,id if ignoreId else None)
